...
# Script to build all packages using tsc directly instead of tsup, with type-checking disabled
...
EOF
  ...
  # Build using tsc with type-checking disabled
  echo "Running tsc for @elizaos/${pkg}..."
  tsc --skipLibCheck --skipDefaultLibCheck --noEmitOnError -p tsconfig.build.json
