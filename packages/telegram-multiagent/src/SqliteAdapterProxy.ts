...
// Import the logger using lowercase name to match the existing code style
const elizaLogger: ElizaLogger = console;
...
export class SqliteDatabaseAdapter {
  ...
  private initializeSchema(): void {
    ...
    try {
      ...
      console.log(`[SQLITE] SqliteDatabaseAdapter: Initialized ${tables.length} tables: ${tables.map((t: any) => t.name).join(', ')}`);
   ...
   */
  async queryOne(sql: string, params: unknown[] = []): Promise<unknown | null> {
    await this.ensureReady();
...
// Define interface for adapter methods
interface SqliteAdapter {
  query?: (sql: string, params: unknown[]) => Promise<unknown>;
  execute?: (sql: string, params: unknown[]) => Promise<unknown>;
  createMemory?: (content: unknown) => Promise<unknown>;
  searchMemories?: (query: unknown, limit: number) => Promise<unknown[]>;
  saveMemory?: (memory: unknown) => Promise<void>;
  testSqliteConnection?: () => Promise<boolean>;
}
...
export class SqliteAdapterProxy {
  ...
  constructor(adapter: SqliteAdapter, adapterName: string = 'unknown') {
  ...
  async checkHealth(): Promise<boolean> {
    try {
      ...
      if (this.adapter.testSqliteConnection) {
      ...
      elizaLogger.error(`[SQLITE_PROXY] Health check for ${this.adapterName} failed: ${error instanceof Error ? error.message : JSON.stringify(error)}`, '', '');
   ...
   */
  private logSqliteError(method: string, error: unknown, params?: unknown): void {
    elizaLogger.error(`[SQLITE_PROXY] Error in ${this.adapterName}.${method}: ${error instanceof Error ? error.message : JSON.stringify(error)}`, '', '');
    if (error instanceof Error && 'code' in error) {
      elizaLogger.error(`[SQLITE_PROXY] SQLite error code: ${(error as any).code}`, '', '');
    }
    if (error instanceof Error && 'errno' in error) {
      elizaLogger.error(`[SQLITE_PROXY] SQLite errno: ${(error as any).errno}`, '', '');
    }
    if (params) {
  ...
  async query(sql: string, params: unknown[] = []): Promise<unknown> {
    ...
    try {
      ...
      this.shouldOpenCircuitBreaker(error instanceof Error ? error : new Error(String(error)));
   ...
   */
  async createMemory(content: unknown): Promise<unknown> {
    if (this.circuitBreakerOpen) {
   ...
   */
  async searchMemories(query: unknown, limit: number = 5): Promise<unknown[]> {
    if (this.circuitBreakerOpen) {
